...
/*function UlkelerinKazanmaSayilari(dizi, callback) {
  const finaller = callback(dizi);

  const kazananlar = finaller.reduce((checkWinner) => {
    if (checkWinner["Home Team Goals"] > checkWinner["Away Team Goals"]) {
      return checkWinner["Home Team Initials"];
    } else {
      return checkWinner["Away Team Initials"];
    }
  }, []);
  return UlkelerinKazanmaSayilari;
}
